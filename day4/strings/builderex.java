package day4.strings;

public class builderex {
    public void stringInsert() {
        StringBuilder build = new StringBuilder("JA");
        System.out.println("Old String " + build);
        build.insert(1,"AV");
        System.out.println("New string " + build);
    }

    public void stringAppend() {
        StringBuilder buffer = new  StringBuilder("BOBZY");
        System.out.println("Old String " + buffer);
        buffer.append("THE KING");
        System.out.println("New string " + buffer);
    }


    public void stringReverse() {
        StringBuilder buffer = new  StringBuilder("NOHYTP");
        System.out.println("Old String " + buffer);
        buffer.reverse();
        System.out.println("New string " + buffer);
    }

    public static void main(String[] args) {
        new builderex().stringInsert();
        new builderex().stringAppend();
        new builderex().stringReverse();
    }
}
